...
from mytrain import test, adv_test, get_pred
...
# #------------------------------------------------------
# writelog('----------EVAL STANDARD PGD-200 L2 ----------------', logfile)
# writelog('model_dir:{}'.format(model_dir), logfile)
# nat_acc = test(model, test_loader, device)
# adv_acc = adv_test_l2(model, test_loader, device, eval_params_l2)
# writelog('--------------------------', logfile)
# for key in eval_params_l2.keys(): 
#     writelog('eval_params, {}:{}'.format(key, eval_params_l2[key]), logfile)
...
if args.eval_multi:
    ...
    for eps in epsilon_range:
        ...
        eval_multi['num_steps'] = 200 # CHANGE HERE 50 to 200
        # nat_acc = test(model, test_loader, device)
...
#------------------------------------------------------
if args.eval_multi_l2:
    writelog('----------EVAL MULTIPLE EPSILONS - NORM L2 ----------------', logfile)
    writelog('model_dir:{}'.format(model_dir), logfile)
    eval_multi = eval_params_l2.copy()
    for eps in epsilon_l2_range:
        eval_multi['epsilon'] = eps
        eval_multi['num_steps'] = 50 
        # nat_acc = test(model, test_loader, device)
        adv_acc = adv_test_l2(model, test_loader, device, eval_multi)
        writelog('--------------------------', logfile)
        for key in eval_multi.keys(): 
            writelog('eval_params, {}:{}'.format(key, eval_multi[key]), logfile)
        writelog('nat_acc={:.4f}, adv_acc={:.4f}'.format(nat_acc, adv_acc), logfile)
...
if args.eval_scan: 
    ...
    for cur_dir in all_dirs:
        ...
        model.eval()    
        nat_acc = test(model, test_loader, device)
        adv_acc = adv_test(model, test_loader, device, eval_scan)
