...
class ImagenetDataset(torchvision.datasets.ImageFolder):
    ...
    def __getitem__(self, index):
        ...
        return (
            (index, sample_transformed, target)
            if not self.ulb
            else (index, sample_transformed, self.strong_transform(sample))
        )
...
class ImageNetLoader:
    ...
    def get_ulb_train_data(self):
        ...
        data = ImagenetDataset(
            root=os.path.join(self.root_path, "train"), transform=transform, ulb=True
        )
        return data
    ...
    def get_lb_test_data(self):
        ...
        data = ImagenetDataset(
            root=os.path.join(self.root_path, "val"), transform=transform, ulb=False
        )
        return data
...
class SSL_Dataset:
        ...
        if "CIFAR" in self.name.upper():
            ...
            if self.train:
                if svhn_extra:  # train+extra
                    ...
                    data_b, targets_b = (
                        dset_base.data.transpose([0, 2, 3, 1]),
                        dset_base.labels,
                    )
                    dset_extra = dset(self.data_dir, split="extra", download=True)
                    data_e, targets_e = (
                        dset_extra.data.transpose([0, 2, 3, 1]),
                        dset_extra.labels,
                    )
            ...
            data, targets = dset_lb.data.transpose([0, 2, 3, 1]), dset_lb.labels.astype(
                np.int64
            )
    ...
    def get_ssl_dset(
        ...
        if self.name.upper() == "STL10":
            ...
            lb_data, lb_targets, _ = sample_labeled_data(
                self.args, lb_data, lb_targets, num_labels, self.num_classes
            )
            ulb_targets = None
        else:
            data, targets = self.get_data()
